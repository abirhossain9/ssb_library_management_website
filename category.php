            ...
            <div class="container">
                <div class="row">
                    ...
                    <?php if (isset($_GET['category'])) {
                        ...
                        $catSelectID = "SELECT * FROM category WHERE cat_name = '$cat_name'";
                        $readCat = mysqli_query($connect, $catSelectID);
                        while ($row = mysqli_fetch_assoc($readCat)) {
                            $cat_id = $row['cat_id'];
                            $cat_name = $row['cat_name'];
                        }

                        $query = "SELECT  * FROM post WHERE category_id = '$cat_id' ORDER BY post_id DESC";
                        $allPosts = mysqli_query($connect, $query);
                        $count = mysqli_num_rows($allPosts);
                        if ($count <= 0) {
                            echo '<div class = "alert alert-info">No post found in this category</div>';
                        } else {
